...
export function TriageLabel(props: NodeProps<TriageNode>) {
  ...
  if (props.data.stepType === "step") {
    return null;
  }

  return (
    <>
      {[TriageTags.Emergency, TriageTags.Delayed, TriageTags.Minor].map(
        (tag) => {
          return (
            <div
              className={clsx({
                "flex items-center justify-center w-full h-10 rounded-[10px] px-2 py-1 text-black font-bold":
                  true,
